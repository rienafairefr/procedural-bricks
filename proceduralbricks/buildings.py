...
class WindowWall(ElementGroup):
    def __init__(self, start, width, facing=Facing.FRONT, connections=Connections()):
        ...
        self.append(Wall((start[0], start[1], start[2]), (start[0] + 40, start[1] + 6 * 24, start[2]),
        ...
        while x < start[facing.x] + width * 40 - 40:
            if last_solid:
                ...
                if random() > .5:
                    ...
                    self.append(Wall((start[0] + x, start[1] + 4 * 24, start[2]),
...
class WallBox(ElementGroup):
    def __init__(self, a, b, Wall_cls, front=True, facing=Facing.FRONT, connections=Connections()):
        ...
        if front:
            self.append(Wall_cls(a, (b[0], b[1], a[2]), facing=Facing.FRONT,
                                 connections=Connections(left=BRICK_EVEN, right=BRICK_EVEN)))
        self.append(Wall_cls(a, (a[0], b[1], b[2]), facing=Facing.LEFT,
                             connections=Connections(left=BRICK_ODD, right=BRICK_ODD)))
        self.append(Wall_cls((a[0], a[1], b[2] - 20), (b[0], b[1], b[2] - 20), facing=Facing.BACK,
                             connections=Connections(left=BRICK_EVEN, right=BRICK_EVEN)))
        self.append(Wall_cls((b[0] - 20, a[1], a[2]), (b[0] - 20, b[1], b[2]), facing=Facing.RIGHT,
                             connections=Connections(left=BRICK_ODD, right=BRICK_ODD)))
...
class ModularBuilding(ElementGroup):
    def __init__(self):
        ...
        for y in range(0, 8 * 8 * 24, 8 * 24):
            ...
            self.append(
                Wall((0, y, 0), (31 * 40, y + 24 * 2, 0), connections=Connections(left=BRICK_EVEN, right=BRICK_EVEN)))
